...
export class Section extends Component<Props> {
  forwardedRef = createRef<HTMLDivElement>();
  scrollable: boolean;
  ...
  componentDidMount() {
    ...
    if (!this.forwardedRef?.current) return;
    if (!scrollable && !scrollableHorizontal) return;

    addScrollableNode(this.forwardedRef.current);
  }

  componentWillUnmount() {
    if (!this.forwardedRef?.current) return;

    removeScrollableNode(this.forwardedRef.current);
  }
