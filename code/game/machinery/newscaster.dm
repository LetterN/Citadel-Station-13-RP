...
/datum/feed_network/proc/alert_readers(annoncement)
	...
	spawn(0) // i beg you async this please
...
/obj/machinery/newscaster
	...
	var/unit_no = 0 // unique id
	var/alert_delay = 500
	var/alert = FALSE
	var/scanned_user = "Unknown" //Will contain the name of the person who currently uses the newscaster
	var/msg = ""  //Feed message
	var/datum/news_photo/photo_data = null //ew
	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			...
			if(1)
				...
				if(!news_network.network_channels.len)
			...
			if(3)
				...
				dat+="<B><A href='?src=[REF(src)];set_new_message=1'>Message Body</A>:</B> <BR>[msg]<BR>" //<font face=\"[PEN_FONT]\">[parsemarkdown(msg, user)]</font>
			...
			if(7)
				...
				for(var/datum/feed_channel/FC in news_network.network_channels)
					if(FC.author == "\[REDACTED\]")
						existing_authors += FC.backup_author //news_network.redactedText
					else
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
			...
			var/temp_message = sanitize(usr, "Write your Feed story", "Network Channel Handler", msg)
			if(temp_message)
				msg = temp_message
				updateUsrDialog()
			updateUsrDialog()
