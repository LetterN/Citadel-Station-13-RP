...
GLOBAL_LIST_INIT(freqtospan, list(
	...
	"[FREQ_SYNDICATE]" = "syndradio", // called Mercenary here, but still same css calss
	"[FREQ_CENTCOM]" = "centcomradio",
	"[FREQ_CTF_RED]" = "redteamradio",
	"[FREQ_CTF_BLUE]" = "blueteamradio",
	// <VStation>
	"[FREQ_RAIDER]" = "syndradio", // same red span
	"[FREQ_ENTERTAIN]" = "entertain"
	// </VStation>
	))
...
#define ENCODE_HTML_EPHASIS(input, char, html, varname) \
	var/static/regex/##varname = regex("[char]{1}(.+?)[char]{2}", "g");\
...
/atom/movable/virtualspeaker/Initialize(mapload, atom/movable/M, radio)
	...
	if(ishuman(M))
		...
		job = "AI"
	else if(istype(M, /mob/living/silicon/robot))  // Cyborg
