...
GLOBAL_LIST_INIT(department_radio_keys, list(
	...
	//MODE_KEY_R_EAR = MODE_R_EAR,
	//MODE_KEY_L_EAR = MODE_L_EAR,
	...
	// <VStation>
	RADIO_KEY_MERC = RADIO_CHANNEL_MERC,
	RADIO_KEY_RAIDER = RADIO_CHANNEL_RAIDER,
	RADIO_KEY_EXPLORER = RADIO_CHANNEL_EXPLORER,
	RADIO_KEY_SPECIAL = RADIO_CHANNEL_SPECIAL, //		".+" = "special", //activate radio-specific special functions
	// </VStation>
	...
	// <VStation>
	"å" = RADIO_CHANNEL_MERC, //same key, :t
	// "" = RADIO_CHANNEL_RAIDER,
	// "" = RADIO_CHANNEL_EXPLORER,
	// "" = RADIO_CHANNEL_SPECIAL,
	// </VStation>
...
/mob/living/say(message, bubble_type, list/spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null)
	...
	*/
	...
	var/message_mode = get_message_mode(message) //parse_message_mode(message, "headset")
	...
	//Parse the language code and consume it (rp lang system)
	if(!speaking)
	...
	if((InCritical() && !fullcrit) || message_mode == MODE_WHISPER)
		...
		if(fullcrit)
			...
			message = copytext_char(message, 1, health_diff) + "[message_len > health_diff ? "-.." : "..."]" //it's based on-..
...
/mob/living/send_speech(message, message_range = 6, obj/source = src, bubble_type = bubble_icon, list/spans, datum/language/message_language=null, message_mode)
	...
	var/image/I = image('icons/mob/talk_vr.dmi', src, "[bubble_type][say_test(message)]", FLY_LAYER)
...
/mob/living/proc/binarycheck()
	...
	if (H.l_ear || H.r_ear)
		...
		if(istype(H.l_ear, /obj/item/radio/headset))
			...
			dongle = H.r_ear
		if(dongle?.translate_binary) //this is a fucking crime
			return 1
*/
...
/mob/living/proc/treat_message(message)
	...
	if((HULK in mutations) && health >= 25) //VStation specific.
		message = "[uppertext(message)]!!!"
