...
/obj/machinery/power/am_control_unit/process()
	...
	power_cycle++ //HEY! process() fires every 2 ticks, therefore it takes 8 ticks per power produced
...
/obj/machinery/power/am_control_unit/emp_act(severity)
	switch(severity)
		if(1)
			if(active && prob(25)) // RNG&trade; Compliant
				toggle_power()
...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	if(!user)
		return
	if(W.is_wrench() && istype(W, /obj/item/tool/wrench))
		if(!anchored)
...
/obj/machinery/power/am_control_unit/proc/check_shield_icons()//Forces icon_update for all shields
	...
	if(update_shield_icons == 2)//2 means to clear everything and rebuild
		for(var/obj/machinery/am_shielding/AMS in linked_shielding)
			...
			addtimer(CALLBACK(AMS, .proc/controllerscan), 10)
		...
		for(var/obj/machinery/am_shielding/AMS in linked_shielding)
			AMS.update_icon()
	spawn(20)
		shield_icon_delay = 0 //we don't have that set var thing for this to be callback
	return
...
/obj/machinery/power/am_control_unit/interact(mob/user)
	...
	if(!fueljar)
		...
		dat += "- <A href='?src=[REF(src)];strengthdown=1'>Decrease</A>|<A href='?src=[REF(src)];strengthup=1'>Increase</A><BR><BR>"

	var/datum/browser/popup = new(user, "AMcontrol", "Engine Computer", 420, 500)
